...
use eth_prototype::{utils, message, mac};
...
fn communicate() {
    ...
    let mut init_msg = utils::create_auth_eip8(&remote_id, &private_key, &nonce, &ephemeral_privkey, &pad);
    ...
    dbg!(body.len());
    let ret = utils::parse_body(&body.to_vec(), &mut ingress_mac, &mut ingress_aes, next_size);
