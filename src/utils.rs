...
pub fn read_message(stream: &mut std::net::TcpStream, ingress_mac: &mut mac::MAC, ingress_aes: &mut Aes256Ctr64BE) -> Vec<u8> {
    let mut buf = [0u8; 32];
...
mod tests {
    ...
    fn test_parse_header() {
        let data = hex::decode("2f58e3c6bb1d5de6d29dd182b088b41c07da694b1c6eb0089168fa515a38de21").unwrap();
        ...
        let secret = hex::decode("0e4dcf62fddcd340f169f75321fa25762d827d3d3729e3865160952d50ae9980").unwrap();
    ...
    #[test]
    fn test_setup_frame() {
        let remote_nonce = hex::decode("2402e143d3c04ce598b723d3ba2d0d757b0bec66e4087ee21faa7fd1dd2bd590").unwrap();
        let nonce = hex::decode("7ffc0228f4a7d5b853a6dc301691561436b3c68c62c44f4ffa456c54dab7611f").unwrap();
        let ephemeral_shared_secret = hex::decode("184620303882579dbeb558c042989a8a0f00beb6bdf5f90882f133f676788df7").unwrap();
        ...
        assert_eq!(h_nonce.to_vec(), hex::decode("4fcce44ac9e5c335cb369eab56581fd6ca58d209bb86f25f8372c546163d15bf").unwrap());
        ...
        assert_eq!(shared_secret.to_vec(), hex::decode("506874951bc21614d35452ca1f115da71d7f7250424eec53d189ea8c64ae9843").unwrap());
        ...
        assert_eq!(aes_secret.to_vec(), hex::decode("983509e7b115aaf9835802864eb974c85e89a793212d6542338a510b7366dbba").unwrap());
        ...
        hasher.update(aes_secret);
        let mac_secret = hasher.finalize();
        assert_eq!(mac_secret.to_vec(), hex::decode("3b8864f7c1fe60f6c7cd6079e26545e1fdc7cda84ab04e64d82a9cb465ef5009").unwrap());
    ...
    fn test_decode_rlp() {
        ...
        assert_eq!(decoded[1], hex::decode("ad28cc8c578fc07e766a1d0aaf6372b57a05eafc051b4e2d4fff17bc886d320f").unwrap());
    }

    #[test]
    fn test_ecdh_x() {
        let ephemeral_private_key = hex::decode("645fb05f68f5401fb1cd05f7cc3b2d4a04568e95a258ed0a01e7e3c91cce6c59").unwrap();
        ...
        assert_eq!(ephemeral_shared_secret, hex::decode("8467b2a5e12124c7825466d59e10c09981ce25172bde9864853a081b12412267").unwrap());
    }
